...
//           </>
//         )}
//         {/* Button below the Route Notes */}
//         <button className={styles.routeButton} onClick={() => alert('Button clicked!')}>
//               Click Me
//             </button>
//       </div>
...
function MapView({ location }) {
  ...
  const handleUpdateStatus = async () => {
    const routeId = routeDetails ? routeDetails.id : 1; // Replace with actual routeId if needed
    ...
    try {
      const response = await fetch(
        `https://localhost:7046/api/Route/UpdateStopOrderAndStatus?RouteId=${routeId}`,
        {
          ...
          body: JSON.stringify({
            // Add any required data here, e.g., updated stop order or status
          }),
        }
      );
      ...
      if (data.statusCode === 200 && data.isSuccess) {
        ...
        // Optionally, update the route details or stop list if needed
        fetchRouteDetails(driverId); // Refetch route details if necessary
      } else {
    ...
    <div className={styles.container}>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {routeDetails.routeStops
                  .filter((stop) => stop.stopOrder > 0) // Chỉ hiển thị các stop có stopOrder > 0
                  .map((stop) => (
                    <tr key={stop.id}>
                      <td>{stop.stopOrder}</td>
                      <td>{stop.address}</td>
      ...
      <div className={styles.map}>
        ...
        >
          ...
          {stopCoordinates
            .filter((stop) => stop.stopOrder > 0 && stop.latitude && stop.longitude) // Lọc các stops có stopOrder > 0
            .map((stop) => (
              <Marker key={stop.id} position={[stop.latitude, stop.longitude]}>
                <Popup>
                  <strong>Stop Order:</strong> {stop.stopOrder}<br />
